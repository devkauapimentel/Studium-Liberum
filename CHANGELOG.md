...
- **Turbopack Compilation Hang (52s)**: Corrigido bug crítico onde os bancos de dados do SQLite (`.studium.db` e um arquivo órfão `.studium-search.db` de 217MB) estavam sendo gerados na raiz do projeto (não ignorados pelo `.gitignore`), fazendo com que o *File Watcher* do Next.js tentasse parsear os binários massivos a cada `npm run dev`. Os arquivos foram movidos para a pasta `/data/` e os caminhos refatorados em `src/lib/db.ts`, resultando em builds instantâneos (300ms).
- **Absorção 42.rioPreparation**: O app isolado da Piscina da 42 foi completamente absorvido pelo Studium Liberum. Rotas e componentes renderizados em `/track/42-prep`. Markdowns redundantes deletados (lidos nativamente de `library/42-prep/curriculum`). Estado migrado de `localStorage` para persistência global em banco de dados SQLite (`.studium.db`).
