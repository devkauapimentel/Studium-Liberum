    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 overflow-y-auto" style={{ marginLeft: "var(--sidebar-width)", minHeight: "100vh" }}>
        ...
        <div className="p-8 space-y-8 animate-fade-in">
          ...
          <Link href={config.tracks.length > 0 ? `/track/${config.tracks[0].id}` : "/settings"} className="block">
            <section className="glass-card p-6 animate-pulse-glow group" style={{ borderLeft: "4px solid var(--color-accent-blue)" }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs text-[var(--color-accent-blue)] font-semibold uppercase tracking-wider mb-1">Continuar de onde parou</p>
                    <h3 className="text-lg font-bold group-hover:text-white transition-colors">
                      {totalFiles > 0 ? "Retomar último estudo" : "Adicione seu primeiro material de estudo"}
                    </h3>
          ...
          <section className="grid grid-cols-4 gap-4">
            ...
            <StatCard icon="📚" value={config.tracks.length} label="Tracks" color="var(--color-accent-green)" />
          </section>
          ...
          <section className="grid grid-cols-3 gap-6">
            ...
            <div className="glass-card p-6 flex items-center justify-center">
              <ProgressRing value={totalFiles > 0 ? Math.min((totalFiles / 100) * 100, 100) : 0} size={140} strokeWidth={10} color="var(--color-accent-purple)" label="Biblioteca" sublabel={`${totalFiles} materiais indexados`} />
            </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-4">
              {config.tracks.map((track) => {
                ...
                return <TrackCard key={track.id} track={track} fileCount={stats} href={`/track/${track.id}`} />;
              })}
            </div>
          </section>
        </div>
